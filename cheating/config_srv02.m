...
% AMP_TYPE          Power Amplifier type: 'UPM_1503', 'UPM_2405', or 'Q3'
...
% VMAX_AMP  Maximum Amplifier Output Voltage                    (V)
% IMAX_AMP  Maximum Amplifier Output Current                    (A)
%
% Copyright (C) 2008 Quanser Consulting Inc.
% Quanser Consulting Inc.
%%
% 
function [ Rm, kt, km, Kg, eta_g, Beq, Jm, Jeq, eta_m, K_POT, K_TACH, K_ENC, VMAX_UPM, IMAX_UPM ] = config_srv02( EXT_GEAR_CONFIG, ENCODER_TYPE, TACH_OPTION, AMP_TYPE, LOAD_TYPE )
    ...
    [K_POT, K_TACH, K_ENC, VMAX_UPM, IMAX_UPM] = srv02_calib ( ENCODER_TYPE, AMP_TYPE, Kg );
...
function [ Kg, Beq, Jeq ] = calc_eq_damping_inerta( EXT_GEAR_CONFIG, LOAD_TYPE, Kgi, eta_g, Jm, J24, J72, J120 )    
    ...
    elseif strcmp (LOAD_TYPE, 'CNCNom')
        % J = mr^2 / 2
        Jl_ext = 1.49e-03;
...
function [K_POT, K_TACH, K_ENC, VMAX_UPM, IMAX_UPM] = srv02_calib ( ENCODER_TYPE, AMP_TYPE, Kg )
    % Analog input range is +/ 2.5 V
    if strcmp(AMP_TYPE,'Q3')
        % Potentiometer Sensitivity (rd/V)
        K_POT = -(352 * pi / 180 / 5);
        % Tachometer Sensitivity (rd/s/V)
        K_TACH = -2*(1000 * 2 * pi / 60 / 1.5 / Kg);
    % For UPM, analog input range is +/ 5 V
    else        
        % Potentiometer Sensitivity (rd/V)
        K_POT = -(352 * pi / 180 / 10);
        % Tachometer Sensitivity (rd/s/V)
    ...
    %
    % Set maximum output voltage and current of amplifier
    if  strcmp (AMP_TYPE, 'UPM_2405')
        VMAX_UPM = 24;
        IMAX_UPM = 5;
